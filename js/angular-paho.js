...
(function() {
  angular.module('angularPaho').factory('MqttClient', ['$rootScope', function($rootScope) {
    ...
    client.on = function(topic, options, callback){
	callbacks[topic] = callback;
	subscribe(topic, options);
    }
    client.onConnectionLost = function(callback){
	_connectionLostCb = callback;
    }
    ...
    function onConnectionLost(resp) {
      //console.log("[DEBUG] Connection lost on ", client._id, ", error code: ", resp);
      client._isConnected = false;
      $rootScope.$apply(function() {
	_connectionLostCb(resp);
      });
    }
    ...
    function onMessageArrived(message) {
      //console.log("[DEBUG] onMessageArrived: ", message);
      angular.forEach(callbacks, function(callback, name){
	if(name === message.destinationName){
	  $rootScope.$apply(function() {
		callback(message);
	  });
	}
